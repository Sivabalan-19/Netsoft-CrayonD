...
function Pricingpop({ setOpenPricingDialog }) {
  ...
  const listofpri = [
    ...
    { sno: 3, conte: "One Time Charges", primcolo: "#6DAFB3", secodcol: "#DBF0F180" },
    { sno: 4, conte: "Refundables", primcolo: "#6D80B3", secodcol: "#E4EDFF80" },
    { sno: 5, conte: "Inventory Item", primcolo: "#B3A16D", secodcol: "#FFFAD880" },
    { sno: 6, conte: "Parking Slot", primcolo: "#B3776D", secodcol: "#FEEAEA80" },
  ];

  const handleCardClick = (item) => {
    setSelectedItem(item);
    if (item.sno === 1) {
      setOpenOtherPopup(true); // Open the other popup if sno is 1
    } else {
      setOpenPopup(true); // Open the regular popup otherwise
    }
  };
  ...
  return (
    <Box className="pricepopcon">
          ...
          >
            <Box className="priciintd" style={{ gap: "5%" }}>
              <Box className="pricpopcir">
                <span style={{ backgroundColor: item.primcolo }}>0{item.sno}</span>
              </Box>
      ...
      {/* Regular Popup Dialog */}
      <Dialog open={openPopup} onClose={() => setOpenPopup(false)} PaperProps={{
          style: { height: "550px", width: "500px" },
        }}>
        <InnerPricingpopup selectedItem={selectedItem} setOpenPopup={setOpenPopup} />
      </Dialog>

      {/* Other Popup Dialog */}
      <Dialog open={openOtherPopup} onClose={() => setOpenOtherPopup(false)} PaperProps={{
          style: { height: "550px", width: "500px" },
        }}>
        <PrimaryInener selectedItem={selectedItem} setOpenOtherPopup={setOpenOtherPopup} />
      </Dialog>
    </Box>
  );
}
